...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # ==========================
        E_x = inputs.mean(-1,keepdim=True)
        Var_x = inputs.var(-1, keepdim=True, unbiased=False) 
        # ==========================

        return (inputs - E_x)*self.weight/(torch.sqrt(Var_x + self.eps)) + self.bias
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length):
        ...
        # ==========================
        self.Weight_Q = nn.Parameter(torch.Tensor(num_heads*head_size, num_heads*head_size))
        self.Weight_V = nn.Parameter(torch.Tensor(num_heads*head_size, num_heads*head_size))
        self.Weight_K = nn.Parameter(torch.Tensor(num_heads*head_size, num_heads*head_size))
        
        self.bias_Q = nn.Parameter(torch.Tensor(num_heads*head_size))
        self.bias_V = nn.Parameter(torch.Tensor(num_heads*head_size))
        self.bias_K = nn.Parameter(torch.Tensor(num_heads*head_size))
        
        self.Weight_O = nn.Parameter(torch.Tensor(num_heads*head_size,num_heads*head_size))
        self.bias_O = nn.Parameter(torch.Tensor(num_heads*head_size))
        
        self.weights = [self.Weight_Q, self.Weight_K, self.Weight_V, self.Weight_O]
        self.biases = [self.bias_Q, self.bias_K, self.bias_V, self.bias_O]
    ...
    def get_attention_weights(self, queries, keys):
        ...
        product = torch.matmul(queries, keys_transposed)
        weights = (product / np.sqrt(self.head_size))
        # ==========================
    ...
    def apply_attention(self, queries, keys, values):
        ...
        hi = torch.matmul(attention_weights,values)
        hi_resized = hi.permute(0,2,1,3)
        # ==========================

        return hi_resized.contiguous().view(hi_resized.size()[0], hi_resized.size()[1], hi_resized.size()[2]*hi_resized.size()[3])
    ...
    def split_heads(self, tensor):
        ...
        return tensor.view(tensor.shape[0], tensor.shape[1], self.num_heads, int(tensor.shape[2]/self.num_heads)).permute(0,2,1,3)
    ...
    def merge_heads(self, tensor):
        ...
        return temp_tensor.contiguous().view(batch_size, sequence_length, num_heads*dim)
    ...
    def forward(self, hidden_states):
        ...
        # ==========================
        X = hidden_states
        Q = torch.matmul(X, self.Weight_Q) + self.bias_Q
        K = torch.matmul(X, self.Weight_K) + self.bias_K
        V = torch.matmul(X, self.Weight_V) + self.bias_V
        concat_h = self.apply_attention(self.split_heads(Q), self.split_heads(K), self.split_heads(V))
        # ==========================

        return torch.matmul(concat_h, self.Weight_O) + self.bias_O
...
class PreNormAttentionBlock(nn.Module):
    ...
    def forward(self, x):
        # ==========================
        output_layer_norm1 = self.layer_norm_1(x)
        output_attn = self.attn(output_layer_norm1)
        output_layer_norm2 = self.layer_norm_2(output_attn + x)
        output = self.linear(output_layer_norm2)
        # ==========================

        return output + output_attn + x
...
class VisionTransformer(nn.Module):
    ...
    def get_patches(self,image, patch_size, flatten_channels=True):
        ...
        # ==========================
        B, C, H, W = image.shape
        patches = image.reshape(B, C, H//patch_size, patch_size, W//patch_size, patch_size)
        patches = patches.permute(0, 2, 4, 1, 3, 5)
        sequence = patches.flatten(1,2)
        if flatten_channels:
            sequence = sequence.flatten(2,4)       
        # ==========================
        return sequence
