...
if __name__ == '__main__':
    ...
    for i in range(n_iter):
        ...
        for _ in range(n_critic_updates):
            ...
            latents = torch.randn([real_images.shape[0], z_dim], device=device)
            fakes = generator(latents).detach().to(device)
            
            loss = vf_wasserstein_distance(real_images, fakes, critic)

            grad_penalty = lp_coeff * lp_reg(real_images, fakes, critic)
        ...
        fakes_score = -vf_wasserstein_distance(real_images, fakes, critic)
    ...
    generator = torch.load('generator_final.pt', map_location='cpu')
    generator.eval()
    critic = torch.load('critic_final.pt', map_location='cpu')
    critic.eval()
    # Generating a samples Q2
    z = torch.randn(64, z_dim, device=device)
    imgs = generator(z)
    save_image(imgs, f'imgs_1.png', normalize=True, value_range=(-1, 1))
    
    # Making perturbations to sample Q3
    z_1 = torch.randn(1, z_dim, device=device)
    z_1_ = np.repeat(z_1, 8, axis = 0)
    for row, dim in enumerate(range(0,100,15)):
        z_1_[row+1][dim] += 5
    
    imgs_1 = generator(z_1_)
    save_image(imgs_1, f'imgs_2.png', normalize=True, value_range=(-1, 1))

    # Compare between interpolating in the data space and in the latend space
    z_2 = torch.randn(2, z_dim, device=device)
    imgs_2 = generator(z_2)
    # Q 4.a
    z_3 = torch.Tensor([z_2.numpy()[0] * (10-i)/10 + z_2.numpy()[1] * (i/10) for i in range(10)])
    imgs_3 = generator(z_3)
    save_image(imgs_3, f'imgs_3.png', normalize=True, value_range=(-1, 1))
    # Q 4.b
    imgs_4 = [imgs_2[0] * (10-i)/10 + imgs_2[1] * (i/10) for i in range(10)]
    save_image(imgs_4, f'imgs_4.png', normalize=True, value_range=(-1, 1))
